...
<head>
    ...
    <link rel="stylesheet" href="scripts/print_style.css" type="text/css" media="print">
    <script language="Javascript" src="scripts/CharDirZoomScroll/cdjcv.js"></script>
    <style>
        div.chartPushButtonSelected2 { padding:5px; background:#ccffcc; cursor:hand; }
        div.chartPushButton2 { padding:5px; cursor:hand; }
        td.chartPushButton2 { font-family:Verdana; font-size:9pt; cursor:pointer; border-bottom:#000000 1px solid; }
    </style>
</head>
...
while($proj_row = mysql_fetch_array($proj_result)){
   ...
   //Limit project names to 24 characters in length
   if(strlen($projName) > 24){
      $projName = substr($projName,0,22);
      $projName = $projName."...";
   }
   ...
   $getMax = "SELECT MAX(wt.due_date) as end, SUM(wt.planned_hours) as total_hours";
   $getMax.= " FROM pev__wbs_to_task AS wt";
   $getMax.= " LEFT JOIN pev__wbs ON pev__wbs.wbs_id=wt.wbs_id";
   $getMax.= " LEFT JOIN pev__project ON pev__project.project_id=pev__wbs.project_id"; 
   $getMax.= " WHERE pev__wbs.project_id=$strProject AND wt.due_date>0";
   ...
   if(isset($max_row['end']) && $max_row['total_hours'] > 0){
      ...
      $getTasks = "SELECT wt.due_date, wt.planned_hours";
      $getTasks.= " FROM pev__wbs_to_task AS wt";
      $getTasks.= " LEFT JOIN pev__wbs ON pev__wbs.wbs_id=wt.wbs_id";
      $getTasks.= " LEFT JOIN pev__project ON pev__project.project_id=pev__wbs.project_id";
      $getTasks.= " WHERE pev__wbs.project_id=$strProject AND wt.due_date>0";
      ...
      $proj_tasks = "SELECT wt.due_date, SUM(wt.planned_hours) AS planned_hours";
      $proj_tasks.= " FROM pev__wbs_to_task AS wt";
      $proj_tasks.= " LEFT JOIN pev__wbs ON pev__wbs.wbs_id=wt.wbs_id";
      $proj_tasks.= " LEFT JOIN pev__project ON pev__wbs.project_id=pev__project.project_id";
      $proj_tasks.= " WHERE pev__wbs.project_id=$strProject AND wt.due_date>0 GROUP BY wt.due_date ORDER BY wt.due_date";
      $proj_tasks_result = dbquery($proj_tasks);

      //Get the project's percent completeness by hours
      $getPercent = "SELECT SUM(wt.planned_hours*(wt.percent_complete/100)/$total_hours) as percent_complete";
      $getPercent.= " FROM pev__wbs_to_task AS wt";
      $getPercent.= " LEFT JOIN pev__wbs ON pev__wbs.wbs_id=wt.wbs_id";
...
//echo "Due Date: " . date("d M Y",$due_date) . ", Cumul. %: $cumulative_percent<br>";
      ...
      if($cumulative_percent - $prev_percent_complete == 0){
         $sched_var_percent = 0;
      }else{
...
/*
//Set graph time frame to +- 1 year
$today = chartTime2(date("U"));

foreach($endDate1 AS $key => $value){
   if($value < $today-(3600*24*365)){
      $endDate1[$key] = $today-(3600*24*365);
   }else if($value > $today+(3600*24*365)){
      $endDate1[$key] = $today+(3600*24*365);
   }
}
...
foreach($startDate1 AS $key => $value){
   if($value < $today-(3600*24*365)){
      $startDate1[$key] = $today-(3600*24*365);
   }
}

foreach($percentStart AS $key => $value){
   if($value < $today-(3600*24*365)){
      $percentStart[$key] = $today-(3600*24*365);
   }
}

foreach($percentEnd AS $key => $value){
   if($value < $today-(3600*24*365)){
      $percentEnd[$key] = $today-(3600*24*365);
   }else if($value > $today+(3600*24*365)){
      $percentEnd[$key] = $today+(3600*24*365);
   }
}
...
$today = date("U");
if($chartStart < $today-(3600*24*365)){
   $chartStart = $today-(3600*24*365);
}

if($chartEnd > $today+(3600*24*365)){
   $chartEnd = $today+(3600*24*365);
}
*/
...
$textBoxObj = $c->addTitle("Status Overview for SoftEng", "timesbi.ttf", 15, 0xffffff);
...
//$c->yAxis->setLabelStep(4);
   ...
   if(!$strSmall){
      ...
      $b = $c->addLegend(825, 25, false, "arialbd.ttf", 8);
    ...
    $imageMap = $c->getHTMLImageMap("show.php", "",
        "title='{xLabel}'");
...
<table cellSpacing="0" cellPadding="0" border="0">
    ...
    <tr valign="top">
        <td class="noprint" style="border-left:black 1px solid; border-right:black 1px solid; border-bottom:black 1px solid;" width="150" bgColor="#c0c0ff">
        ...
        <td>
            <div class="noprint" style="font-weight:bold; font-size:20pt; margin:5px 0px 0px 5px; font-family:Arial">
                Software Engineering Project Status
            </div>
            <hr class="noprint" color="#000088">
