...
if($actual_cost == 0 && $earned_value == 0){
   //Handle div0 warning
   $CPIvalue = 1;
}elseif($actual_cost == 0 && $earned_value > 0){
   //Don't want infinite number or div0
   $CPIvalue = 2;
}else{
   $CPIvalue = $earned_value/$actual_cost;
}
...
if($CPIvalue>= 0.9 && $CPIvalue<=1.3)$bgC=0xDDFFDD;
else if($CPIvalue>=0.5 && $CPIvalue<0.9 || $CPIvalue>1.3 && $CPIvalue<=1.7)$bgC=0xFFFFDD;
else $bgC=0xFFDDDD;
